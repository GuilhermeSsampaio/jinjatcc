...
export default function PaginaUsuarioLista() {
  ...
  useEffect(() => {
    async function carregarItens() {
      try {
        setLoading(true);
        const resposta = await servicoListarUsuario();
        setItens(Array.isArray(resposta) ? resposta : (resposta?.data || []));
      } catch (erro) {
        mostrarToast(toastRef, "Erro ao carregar usuarios", "erro");
      } finally {
        setLoading(false);
      }
    }
  ...
  async function handleRemoverConfirmado() {
    try {
      await servicoRemoverUsuario(itemSelecionado.id);
    ...
    return (
      <div className={estilizarDivBotoesAcao()}>
        <Button
          ...
          tooltip="Editar"
          tooltipOptions={{ position: "top" }}          onClick={() => navigate(`/usuarios/${rowData.id}/editar`)}
        />
        <Button
          icon="pi pi-trash"
