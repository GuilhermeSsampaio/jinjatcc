...
import {
  servicoListarPedido,
  servicoRemoverPedido,
} from "../../servicos/servicos_pedido";
import {
  estilizarBotao,
...
export default function PaginaPedidoLista() {
  ...
  useEffect(() => {
    async function carregarItens() {
      try {
        setLoading(true);
        const resposta = await servicoListarPedido();
        setItens(Array.isArray(resposta) ? resposta : (resposta?.data || []));
      } catch (erro) {
        mostrarToast(toastRef, "Erro ao carregar pedidos", "erro");
      } finally {
        setLoading(false);
      }
    }
  ...
  async function handleRemoverConfirmado() {
    try {
      await servicoRemoverPedido(itemSelecionado.id);
    ...
    return (
      <div className={estilizarDivBotoesAcao()}>
        <Button
          ...
          tooltip="Editar"
          tooltipOptions={{ position: "top" }}          onClick={() => navigate(`/pedidos/${rowData.id}/editar`)}
        />
        <Button
          icon="pi pi-trash"
