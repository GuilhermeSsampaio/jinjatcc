...
export default class ServicosPedido {
  static async listar(request, response) {
    try {
      const itens = await Pedido.find();
  ...
  static async buscarPorId(request, response) {
    try {
      const item = await Pedido.findOne(request.params.id);
      if (!item) {
        return response
          .status(404)
  ...
  static async criar(request, response) {
    try {
      ...
      const novo = Pedido.create(dados);
      const salvo = await Pedido.save(novo);
  ...
  static async atualizar(request, response) {
    try {
      ...
      const dados = request.body;
      const existente = await Pedido.findOne(id);
      if (!existente) {
        return response
          .status(404)
          .json({ mensagem: "Registro nao encontrado" });
      }
      const atualizado = Pedido.merge(existente, dados);
      const salvo = await Pedido.save(atualizado);
